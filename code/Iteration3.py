...
Iteration 3: Added a portal system that allows path to be jumped across gridspace
"""
...
def algorithm(draw, grid, start, end, portals):
    count = 0
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:
                    portals = check_portal(grid)
                    for row in grid:
                        for spot in row:
                            spot.update_neighbors(grid)
                    add_portal_neighbors(portals)       

                    algorithm(lambda: draw(win, grid, rows, width), grid, start, end, portals)
